...
### API and access
22. Every game endpoint requires a signed-in player: a Firebase ID token (Sign in with Google) in `Authorization: Bearer`. *(Revised in slice 15; it was username-only login, intentionally not secure. See DECISIONS 34.)* The username is the only public identity; the Google name and email are never shown or returned. Usernames are 3 to 40 printable ASCII characters (no spaces) and case-insensitive: they are lowercased when chosen, so `Joe` and `joe` are the same player. Anything else is rejected with `invalid_username`. A new account chooses a username on first sign-in; a pre-Google player can link their old username once (first come, first served). A local `AUTH_MODE=dev` still accepts `X-Username` and is refused in production.
...
4. Is username in a header acceptable as "auth"? **Was yes; replaced by Sign in with Google in slice 15** (DECISIONS 34). Usernames are 3 to 40 ASCII characters and case-insensitive (DECISIONS 19).
