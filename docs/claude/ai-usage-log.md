...
### 12. Reviewer role: a numbered audit I could triage in one line per item
- **Phase:** Review
- **Prompt:** "follow instructions in docs/claude. you are a REVIEWER." plus the reviewer kickoff prompt ("You are reviewing, not building. [...] Report: (1) SPEC requirements not met or only partially met; (2) bugs and unhandled edge cases; (3) missing or weak tests; (4) dead code; (5) README accuracy. Don't change code; produce a prioritized list with file references."), then, after an interrupt, "don't do a fresh clone check". I answered the numbered report with one line per item ("6. this is okay. defend and document it", "8. compute final state without ticking", "12. defer it, out of scope", "16. tbd").
- **Why it works:** Role separation ("reviewing, not building", no code changes) gets an audit instead of a rewrite, and a fixed five-part report with file links gives every finding a number. The numbers let me decide in a few words per item: fix, defend and document, defer, or out of scope. The interrupt trimmed the kickoff prompt to what I wanted that round.
- **Result:** About 25 findings. Real ones: the forced-sale bankruptcy rule contradicted the SPEC and pointed to a DECISIONS entry that did not exist; balance numbers were out of sync across documents; a username rule with no length or case policy; `Expand` silently ignoring an unknown facility type; a bankrupt end of day still advancing the market. All were fixed or documented, and each accepted trade-off became a DECISIONS entry ("defend and document").
- **Failure I corrected:** part of my first report was stale. A balance commit had landed while the reviewer was reading, so two "missing" items (the README physics section, the DECISIONS entry) were already done. The model had read the docs early and code later. A re-review after "changes complete" caught the drift, and the second pass also found duplicate DECISIONS numbers.
- **Lesson:** Number every finding and let the reviewer's report be the reply surface. Tell a reviewer to re-read a file before citing it as missing when someone else is committing in parallel.

### 13. Committing only my hunks from a working tree shared with a parallel session
- **Phase:** Build / Process
- **Prompt:** "commit your changes"
- **Why it works:** Another session was editing the same repo, so the tree held both sets of changes. The model listed the diff hunks before committing and found the other session's timeline test mixed into the same test file as the username edits. It rebuilt a clean version of that file from the committed blob plus its own edits, staged that blob directly, and checked out the staged tree into a temp directory to confirm it passed on its own.
- **Result:** Two commits that each build and pass in isolation, with the other session's work left uncommitted for its owner.
- **Lesson:** `git add -A` in a shared tree publishes someone else's half-done work. When files are mixed, stage a constructed blob and test the staged tree, not the working tree.
