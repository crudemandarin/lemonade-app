...
## 32. Sign in with Google (Firebase Auth) replaces username-only login (slice 15)
...
- **Username is the only public identity** (user's decision). New accounts choose one on first sign-in, never prefilled from Google. The email is stored (private, lowercased is not applied; it is never read back) and appears in no DTO; a test asserts no response or DTO field carries an email or UID.
