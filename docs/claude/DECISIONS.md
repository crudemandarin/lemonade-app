...
## 19. Usernames: 3 to 40 ASCII characters, case-insensitive; header lookup on every request
- **Did:** usernames are trimmed, lowercased, and must be 3 to 40 printable ASCII characters with no spaces (`normalizeUsername` in `internal/api/game.go`, applied to login and to the `X-Username` header). The frontend mirrors the rule and sends the lowercase name.
- **Why case-insensitive and ASCII:** with no password, `Joe`, `joe` and a look-alike Unicode name would otherwise be three players that are easy to confuse, and normalizing to one form removes the confusion. The minimum was first 5 (to rule out trivially guessable names like `a` or `bob`) and was lowered to 3 at the user's request, so short names such as `bob` are allowed again; 40 is the original upper bound.
...
## 31. Balance phase 0: the exploit bots and what the baseline showed (slice 14)
- **Did:** `exploitbots_test.go` adds a spammer (price-blind full batches), a thresholder, a hoarder and an opportunist, built on the real domain calls; `TestBalanceReport` prints them next to the existing bots. No game rule changed and no existing balance band was touched. Baseline table in `numeric-tdd.md` section 9.
- **Deviation from the handoff's bot spec:** the thresholder's buy rule "ask at most 0.9 x base" was implemented literally first and starved (100% bankrupt, the same as an idle player): the ask includes the 10% spread, so it needs the price about 18% under base in three inputs at once, while $30/day of upkeep runs. The main thresholder therefore buys on days when the whole margin is good (at least $10) and stocks up at dips (ask at most base), and the literal version stays in the report as `strict-thr`. Bots also keep three days of upkeep in cash and size expansions by working capital, as the existing grower does; without that they bankrupt themselves at factory scale, which is a bot fault, not a game finding.
- **Diagnosis check (handoff section 4 asks for a stop if it is wrong):** the volume part is right (the spammer beats the careful grower). The waiting part is not: skipping unprofitable days costs upkeep, so thresholders and hoarders do worse than a bot that always produces when the margin is positive, and lemonade cannot be hoarded at all because a warehouse holds exactly one day of production. The section 2 targets assume a skilled bot that beats the spammer by 1.5x to 5x; no bot here does, so those targets need the user's decision before phase 1.

## 32. Market depth: separate buy and sell pressure, marginal-price quotes (slice 14, phase 1)
- **Did:** `Game.BuyPressure` and `SellPressure` (cases the player recently bought and sold, per resource) raise the price of each further case beyond `FreeDepth` (0.3% per case, capped at 60%); half is forgotten each night. Buying only raises the ask and selling only lowers the bid, so a same-day buy-then-sell can never profit (a property test over random trades checks this, along with never-negative pressure, impact-aware cost never below the plain cost, determinism, and a within-free-depth trade costing exactly ask times quantity). `Buy`, `Sell`, the bulk `clamp` (which now buys the most that cash pays for once impact is counted), and forced sales on bankruptcy all use it; forced sales take more cases to raise the same cash. Old saves load with zero pressure (a Postgres test covers a NULL row).
- **API and UI:** the view's `bid`/`ask` are the marginal price of the next case, plus `buyDepthLeft`/`sellDepthLeft`, `buyImpactPercent`/`sellImpactPercent`, and a `trade` ladder of server-priced totals for the bulk amounts (1, 10, 50, 100, all). `GET /api/game/quote` prices any size without trading. The market row labels use the ladder (no client arithmetic; the earlier client-side preview is gone), a title explains average price and slippage, and a warning icon with its words in the label marks a price the player has moved. Glossary gains "Price impact" and "Market depth".
- **Valuation:** net worth, the score and unrealized gain value stock at the plain bid and ignore price impact (user's decision). A player can therefore pad the score by ending a run holding a large stock; revisit if it shows on the leaderboard.
- **Knobs:** the handoff's starting numbers (40 free cases, 1.5% per case) made every bot bankrupt, so free depth 80, slope 0.3%, recovery 50%, cap 60% were chosen by a sweep against the bots (numeric-tdd section 9). No existing balance band was widened; the guard-rail tests pass unchanged.

## 33. Phase 1 results, bot changes and what is still open
- **Bot change:** with impact, the existing careful grower (and the skilled trading bots) went bankrupt by buying full batches into rising prices and adding capacity that could not pay for itself. They now size each batch to the last case that still earns money and expand only when capacity is what limits profit; the price-blind spammer is left as it was, because being price-blind is its point. This is a change to the test bots, not to a guard rail.
- **Against the targets:** the spammer is bankrupt or under $1,000 at day 90 in 100% of games (target: at least 25%) and never beats the skilled bots (0% of seeds); the skilled bots are near $9k at day 60 (target: $6k to $10k). **Not met:** "everything maxed" moves from median day 66 to never (target: day 90 or later with at least half still maxing within 120 days), and capital flattens around $15k from day 60 to day 90, so the late game has little left to work toward. Linear impact makes a maxed factory (800 cases a day) unprofitable at any slope that fixes the exploit, so this is a design trade-off rather than a tuning miss.
- **Open for the user (phase 3 or a new mechanic):** accept a lower ceiling; make depth grow with warehouse level or tier; or lower upkeep on big tiers so the late game still grows. Per the handoff I have not added a mechanic on my own.
- **Layout fixes found on the way:** a level-4 factory overflowed a phone in the facilities panel (the expand and sell buttons sat beside the capacity text), the four header figures could not fit once cash had seven digits, and a long best-score chip could widen the page. All three wrap now.

## 34. Sign in with Google (Firebase Auth) replaces username-only login (slice 15)
- **Did:** the web app signs in with Google through the Firebase JS SDK and sends the ID token as a bearer token. The API verifies it (`internal/auth`, Firebase Admin SDK `firebase.google.com/go/v4`, which builds and verifies on the repo's Go 1.27.1; no credentials file, since verifying needs only Google's public keys), requires provider `google.com` and a verified email, and maps the UID to a user. Revises SPEC rule 22 and decision 19's "auth is intentionally weak" stance. Not `VerifyIDTokenAndCheckRevoked` (one extra network call per request); a token lives an hour, accepted.
- **Username is the only public identity** (user's decision). New accounts choose one on first sign-in, never prefilled from Google. The email is stored lowercased (private; nothing reads it back) and appears in no DTO; a test asserts no response or DTO field carries an email or UID.
- **Claim flow (user's decision):** a legacy player links their old username once, first come, first served, in one conditional update (`WHERE firebase_uid IS NULL`) so two racing claims cannot both win. Check order: the caller's UID already linked, then unknown username, then already claimed. Rate limit 5 tries per 10 minutes per UID, in memory (per instance). The hijack risk of the old "type any name" login is documented in the README, not solved; a claim deadline is Future work.
- **Dev bypass (user's decision):** `AUTH_MODE=dev` accepts `X-Username` and mounts `/api/login`; `auth.Config.Validate` makes startup fail on Cloud Run (`K_SERVICE`) or `APP_ENV=production`, and fail for firebase mode without `FIREBASE_PROJECT_ID`. Both refusals are unit tested. No `both` mode in production. Tests use the fake verifier; the emulator integration test is skipped unless `FIREBASE_AUTH_EMULATOR_HOST` is set.
- **Deviations from the handoff:**
  - `CreateUserWithGame` is unchanged and there is a new `CreateProfile(username, uid, email, game)`, so the dev login path and all existing tests keep working untouched.
  - `SessionService` stays as a small in-memory username signal (AuthService fills it from `GET /api/me`) instead of becoming a computed view, which kept the existing component specs valid; it no longer touches localStorage and clears the old `lemonade.username` key.
  - The runtime config is not a generated file: nginx answers `/config/firebase-config.json` directly (`return 200` with the `FIREBASE_*` env vars substituted, `no-store`), which is the same contract with no startup script. `authDomain` is the request host, and nginx proxies `/__/` to `<project>.firebaseapp.com` so redirect sign-in stays same-origin. `ng serve` uses `src/config/firebase-config.json` (emulator). `npm run check:ngsw` proves the service worker neither caches `/config/**` nor treats `/__/*` as a navigation.
  - The initial bundle budget warning went from 500 kB to 600 kB: the Firebase auth SDK added about 170 kB raw (about 50 kB transferred), total 520 kB raw, 135 kB transferred.
- **Bugs the real-stack run caught:** (1) the 403 `profile_required` redirect also fired on the `GET /api/me` probe, so the guard and the redirect cancelled each other forever and a new account never reached onboarding; the interceptor now leaves `/api/me` alone (spec added). (2) On a brand-new browser profile, a first page load abandoned mid-start left the next load's Firebase startup blocked for good in that browser session, so the app sat on "Loading..." (it recovers on the next visit); startup now stops waiting after 10 s and treats the player as signed out. Neither was visible in unit tests.
- **Popup with redirect fallback:** popup normally; redirect if the popup is blocked or the app runs as an installed PWA (`display-mode: standalone`); closing the popup is not an error. No COOP header is set anywhere (`same-origin` breaks popups).
- **Rollout:** Firebase must be set up before merging (deploy/README runbook), because the deploy ships api and web together and the new api rejects the old header. Old clients land on the sign-in page and legacy players claim their names on first Google sign-in.
- **Log candidate:** the Firebase Auth Emulator plus a dev bypass guarded against production, verified by driving a headless browser through the emulator's fake Google popup (which found the two bugs above).
- **First real-Google test failed with `redirect_uri_mismatch` (my miss):** with `authDomain` = the web host, Google redirects to `https://<web-host>/__/auth/handler`, but Firebase's auto-created OAuth client only allows `<project>.firebaseapp.com`. The runbook did not say to add the host, and the emulator cannot show this. Fixed in the runbook (new step 5: add the host as an authorized JavaScript origin and redirect URI, for every host including custom domains) and made `authDomain` overridable (`FIREBASE_AUTH_DOMAIN`, Terraform `firebase_auth_domain`) for anyone who prefers `<project>.firebaseapp.com` and no console step, at the price of cross-origin redirect sign-in.
