...
- **Not done yet:** `POST /api/game/new` while a run is active does not return 409 `run_active`. Without it, starting a new game over an unfinished one still discards that run with no record. Waiting for the user's go-ahead because it changes existing flows.
